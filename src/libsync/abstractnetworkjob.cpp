...
void AbstractNetworkJob::start()
{
    ...
    const QByteArray parentMetaObjectName = parent() ? parent()->metaObject()->className() : "";
...
{
    _timedout = true;
    qCWarning(lcNetworkJob) << "Network job timeout" << (reply() ? reply()->request().url().toString() : path());
    onTimedOut();
}
...
QString AbstractNetworkJob::replyStatusString() {
    ...
    if (reply()->error() == QNetworkReply::NoError) {
        ...
        const QByteArray enumStr = QMetaEnum::fromType<QNetworkReply::NetworkError>().valueToKey(static_cast<int>(reply()->error()));
        return QStringLiteral("%1 %2").arg(QString::fromUtf8(enumStr), errorString());
    }
}
...
QString networkReplyErrorString(const QNetworkReply &reply)
{
    ...
    return AbstractNetworkJob::tr("Server replied \"%1 %2\" to \"%3 %4\"").arg(QString::number(httpStatus), httpReason, QString::fromUtf8(HttpLogger::requestVerb(reply)), reply.request().url().toDisplayString());
}
