    ...
    const RemoteInfo &serverEntry, const SyncJournalFileRecord &dbEntry, QueryMode recurseQueryServer)
{
    ...
    if (wasDeletedOnClient.first) {
        ...
        connect(job, &RequestEtagJob::finishedWithResult, this, [=](const HttpResult<QString> &etag) {
            ...
            if (!etag || (*etag != QString::fromUtf8(base._etag) && !item->isDirectory()) || _discoveryData->isRenamed(originalPath)) {
                ...
                tmp_recurseQueryServer = *etag == QString::fromUtf8(base._etag) ? ParentNotChanged : NormalQuery;
            }
