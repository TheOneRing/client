    ...
    const Share::Permissions desiredPermissions)
{
    ...
    connect(job, &OcsShareJob::shareJobFinished, this,
        [=](const QJsonDocument &reply) {
            ...
            foreach (const QJsonValue &element, reply.object()[QStringLiteral("ocs")].toObject()[QStringLiteral("data")].toArray()) {
...
QSharedPointer<LinkShare> ShareManager::parseLinkShare(const QJsonObject &data)
{
    ...
    if (data.contains(QStringLiteral("url"))) {
        ...
        url = QUrl(Utility::concatUrlPath(_account->url(), QLatin1String("index.php/s/") + data.value(QStringLiteral("token")).toString()));
    } else {
        QUrlQuery queryArgs;
